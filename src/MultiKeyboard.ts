 ...
 */
export class MultiKeyboard extends Keyboard {
	/**
	 * アルファベット用入力キーのグループ。
	 */
	...
	constructor(param: KeyboardParameterObject) {
		...
		const alphaReader = new SpriteReader(param.scene, "hint_key_alpha", "img_key_alpha");
		const symReader = new SpriteReader(param.scene, "hint_key_sym", "img_key_sym");
